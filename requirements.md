...
Unlike traditional calendars that schedule *when* things happen, this tool budgets *how much* time is allocated to categories. The fundamental constraint is the **Total Available Time** for the selected period (initially **Weekly**, which is **168 hours**). Every hour in that period must be accounted for.
Another requirement is Application should be based on responsive designm so it can be used on mobile devices. eventually I may want to add a mobile app.
## 2. Key Mechanics
...
1.  **Period Setup (The Budget)**
    ...
    *   *Constraint*: `Sum(UserAllocatedEnvelopes) <= TotalPeriodHours`. Any unallocated hours will automatically be assigned to an "Unallocated" envelope. The user can proceed without fully allocating all hours.
    *   *Example*:
        *   Sleep: 56.00
        *   Work: 40.00
        *   Commute: 5.00
